...
df_q1 <-
  mpg %>%
  filter(str_detect(trans, pattern = ".l[3-5]"))
...
df_q2 <-
  mpg %>%
  mutate(
    trans_type = str_extract(trans, pattern = '^(auto|manual)'),
    trans_code = str_extract(trans, pattern = '(?<=\\().+(?=\\))')
  )
df_q2 %>% glimpse
```

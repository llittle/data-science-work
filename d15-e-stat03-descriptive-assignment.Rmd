...
The mean departure delay is 13 minutes
...
FL, WN, F9 all show up on both lists.

- Which carriers differ in the two lists?

UA and VX appear on the median lis but not the mean, while EV and YV
appear on the mean but not the median.

- What do negative values of `dep_delay` mean?

Negative dep_delay values means that the plane left earlier than the scheduled 
depart
...
9E is in the IQR but not the sd, and HA is in the sd but not in the IQR. 

- Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean?

F9 has a SD of ~60 but an IQR of only 20. This likely means that there are some outliers for that
airline where they left very late, and those datapoints are skewing the sd (but not the IQR)
...
```{r ex-cor-line}
slope <- -0.05 # Play with this value; observe the correlation
df_line <-
...
The correlation is positive for all positive slope values and negative for all negative slope values.
...
df_pearson = df_quad %>% summarize(rho = cor(x, y, method = 'pearson'))

df_spearman = df_quad %>% summarize(rho = cor(x, y, method = 'spearman'))
...
Because the function is not monotonic I do not expect either correlation
function to show a high correlation.

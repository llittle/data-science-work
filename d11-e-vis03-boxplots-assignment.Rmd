...
```{r q1-task}
diamonds %>%
  mutate(carat_cut = cut_width(carat, width = 1, boundary = 0)) %>%
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()
```

**Observations**
- The median price and the minimum price of a diamond increase for each increase of 1 carat
- There are many high priced outliers for small diamonds in the [0-2] carat range
...
**Observations**:
- For all of the diamonds, the most common diamonds are ideal diamonds 
with colors of D-G
- For diamonds of over 2 carats, the most common diamonds are Premuim diamonds
with colors of J-G

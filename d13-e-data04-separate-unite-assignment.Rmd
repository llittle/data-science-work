...
__q1__ Use a combination of `unite` and pivoting to turn `genes` into the same
dataframe as `punnett`.

```{r q1-task}
df_q1 <-
  genes %>%
  unite('combo',c(parent1, parent2), sep = '', 
        remove = FALSE) %>%
  pivot_wider(names_from = parent2, values_from = combo )
...
df_q2 <-
  alloys_mod %>%
  pivot_longer(c(-thick, -rep), names_to = 'test', values_to = 'n') %>%
  separate(test, into = c('test', 'angle'), 
           sep = -2, remove = FALSE) %>%
